...
/** @FILE
 * 编译器配置，包括
 * - 警告设置支持，用法：在工程中打开全部警告(/Wall)，在stdafx.h里包含工程用到的所有第三方头文件，把它们包在BD_NO_WARNING_AREA_BEGIN/END里
 * - 几个有用的宏:
 *   - WIDESTRING 编译时把一个字符串转成宽字符串
 *   - STRINGIZE 编译时把一段内容转成字符串
 *   - COMPILE_MESSAGE 编译时输出一段信息，双击可以跳到写这条信息的行
 */
